...
`base_kv_config` 定义底层键值引擎配置。当前统一由 [src/storage/kv_engine/engine_selector.h](../src/storage/kv_engine/engine_selector.h) 的 `ResolveEngine` 进行：
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `mode` | string | 否 | `STATIC` 或 `DYNAMIC`；未配置时按 `value_type` 自动推导（`HYBRID -> DYNAMIC`，其他 -> `STATIC`） |
...
| `index_medium` | string | 是 | 索引介质：`DRAM` / `SSD`（兼容旧字段 `index_type`） |
| `value_medium` | string | 是 | 值介质：`DRAM` / `SSD` / `HYBRID`（兼容旧字段 `value_type`） |
| `DRAM_SIZE` | integer | HYBRID 必填 | DRAM 层字节容量（兼容旧字段 `shmcapacity`） |
| `SSD_SIZE` | integer | HYBRID 必填 | SSD 层字节容量（兼容旧字段 `ssdcapacity`） |
| `capacity` | integer | 非 HYBRID 必填 | 预估条目数 |
| `value_size` | integer | 非 HYBRID 必填 | 定长 value 大小（字节） |

#### 分配器字段（推荐新命名）

| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `allocator_type` | string | `PERSIST_LOOP_SLAB` | `PERSIST_LOOP_SLAB` 或 `R2_SLAB` |

兼容旧字段：

- `value_memory_management=PersistLoopShmMalloc` <-> `allocator_type=PERSIST_LOOP_SLAB`
- `value_memory_management=R2ShmMalloc` <-> `allocator_type=R2_SLAB`

#### 变长 value 字段

| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `value_layout` | string | `FIXED`（非 HYBRID）/`VARIABLE`（HYBRID） | `FIXED` 或 `VARIABLE` |

当前限制：`value_layout=VARIABLE` 仅支持 `HYBRID`（ValueManager 路径），其他组合会在启动阶段报错。

#### 模式与组合约束（显式配置 `mode` 时严格校验）

- `STATIC` 支持：
  - `index=DRAM, value=DRAM`
  - `index=DRAM, value=SSD`
  - `index=SSD, value=SSD`
- `DYNAMIC` 支持：
  - `value=HYBRID` 且 `index=DRAM/SSD`

#### 引擎推导规则

- `value=HYBRID` -> `KVEngineHybrid`
- `value=DRAM/SSD` 且 `index=DRAM` -> `KVEngineExtendibleHash`
- `value=DRAM/SSD` 且 `index=SSD` -> `KVEngineCCEH`

当索引介质为 `SSD` 时，`ResolveEngine` 会自动补默认：

- `io_backend_type=IOURING`
- `queue_cnt=512`
- `page_id_offset=0`

并在缺省时填充 `file_path=<path>/index_cceh.db`。
