...
### 3.1 动态覆盖 (Dynamic Override)

在实际运行时（尤其是在多机训练场景下），可以通过 Python API 动态覆盖配置文件的 `host` 与 `port` 设置：

* **KVClient**: `client.set_ps_config(host, port)`
* **RecStoreEmbeddingBagCollection**: 初始化时传入 `ps_host="x.x.x.x", ps_port=xxxx` 参数

这允许在不修改 `recstore_config.json` 的情况下灵活调整连接目标。
