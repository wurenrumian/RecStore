...
#define _kCacheLineSize (64)

inline constexpr uint64_t kWriteLatencyInNS = 10;
inline uint64_t clflushCount                = 0;
...
inline void clflush(char* data, size_t len) {
  volatile char* ptr = (char*)((unsigned long)data & (~(_kCacheLineSize - 1)));
  mfence();
  for (; ptr < data + len; ptr += _kCacheLineSize) {
    unsigned long etcs =
