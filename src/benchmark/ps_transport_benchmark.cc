...
void PrintSummary(const std::string& transport,
  ...
  const double mean_us = total_us / static_cast<double>(elapsed_us_samples.size());
  ...
  const double ops_per_sec =
      (ops_per_round * total_rounds) / (total_us / 1e6);
...
int main(int argc, char** argv) {
  ...
  CHECK(
      report_mode == "summary" || report_mode == "per_round" ||
      report_mode == "both")
