...
extern "C" bool report(
    ...
    // 5. 检查结果
    if (res != CURLE_OK) {
        std::cerr << "REPORT_LIB ERROR: CURL perform failed: " << curl_easy_strerror(res) << std::endl;
    } else {
        // 获取 HTTP 状态码
        ...
        if (http_code >= 200 && http_code < 300) {
            std::cout << "REPORT_LIB INFO: Data for ID [" << unique_id << "] reported successfully." << std::endl;
            success = true;
        } else {
            std::cerr << "REPORT_LIB ERROR: API failed (HTTP Code: " << http_code << ")." << std::endl;
            std::cerr << "REPORT_LIB ERROR: Server response: " << response_buffer << std::endl;
        }
    }
