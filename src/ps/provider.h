...
public:
  ...
  bool ExistTask() override { return tasklist.size() != 0; }

  int LeftTask() override { return tasklist.size(); }

  long long GetTID() override { return task_id; }

  void ImporvePriority(T task) override { return; }
...
public:
  ...
  bool ExistTask() override { return LeftTask() != 0; }
  ...
  void ImporvePriority(T task) override {
    ...
    std::cout << task << " priority has been improved to level "
              << priority_map[task] << std::endl;
