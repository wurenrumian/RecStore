...
inline std::size_t AlignUp(std::size_t value,
                           std::size_t alignment = kLocalShmAlignment) {
...
inline std::size_t SlotHeadersBytes(uint32_t slot_count) {
  return AlignUp(sizeof(LocalShmSlotHeader) * static_cast<std::size_t>(slot_count));
}
...
inline std::size_t SlotPayloadOffset(uint32_t slot_count,
